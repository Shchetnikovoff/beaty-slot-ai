...
- Роль пользователя: ${context.userRole}`;
  ...
  if (context?.pageData) {
    ...
    pageDataInfo = `\n\n📊 Данные текущей страницы (${pageType}):`;
    ...
    if (tableData) {
      ...
      if (tableData.total !== undefined) {
        pageDataInfo += ` всего ${tableData.total}`;
      }
      if (tableData.rows && tableData.rows.length > 0) {
        pageDataInfo += `, показано ${tableData.rows.length}`;
      }
      if (tableData.selectedIds && tableData.selectedIds.length > 0) {
        pageDataInfo += `, выбрано ${tableData.selectedIds.length}`;
      }
      if (tableData.filters && Object.keys(tableData.filters).length > 0) {
        pageDataInfo += ` (фильтры: ${JSON.stringify(tableData.filters)})`;
      }
    }
    ...
    pageDataInfo += `\n(Эти данные уже доступны — можно использовать их напрямую)`;
  }

  return `Ты — умный AI-агент салона красоты Beauty Slot. Сегодня ${today}.
${pageDataInfo}
${contextInfo}

Ты можешь выполнять многошаговые задачи. Анализируй контекст и самостоятельно выстраивай логику действий.

ДОСТУПНЫЕ ИНСТРУМЕНТЫ:

Навигация:
• navigate — переход на страницу (/dashboard, /dashboard/analytics, /apps/customers, /apps/settings)
• showNotification — показать уведомление пользователю

Получение данных:
• getClients — список клиентов (status: all/active/expired, limit)
• getClientDetails — детали конкретного клиента (clientId)
...
• getFullDashboard — полная сводка

Действия:
• sendBroadcast — рассылка сообщения (audience: all/active/expired, message)
• analyzeClients — анализ клиентов (analysisType: activity/spending/churn_risk/growth)

ПРИНЦИПЫ РАБОТЫ:
1. Если у тебя уже есть нужные данные в контексте (pageData, результаты предыдущих инструментов) — используй их напрямую.
2. Вызывай инструменты когда нужны новые данные или действия. Можно вызывать несколько инструментов подряд для сложных задач.
3. Логически выстраивай цепочку действий: например, сначала navigate на нужную страницу, затем анализ данных.
4. Отвечай кратко, точно и на русском языке.`;
};
