...
export const CategoryRevenueChart: React.FC<CategoryRevenueChartProps> = ({
  ...
  if (error) {
    return (
      <ErrorAlert
        title="Ошибка загрузки выручки по категориям"
        message={error.message || 'Не удалось загрузить выручку по категориям'}
      />
    );
  }
  ...
  return (
    <Stack gap="lg">
      <Group justify="space-between">
        <div>
          ...
          <Text size="sm" c="dimmed">
            Всего: ${totalRevenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}
          </Text>
        </div>
      </Group>
      ...
      <BarChart
        ...
        dataKey="category"
        series={[
          { name: 'revenue', label: 'Выручка', color: 'blue.6' },
        ]}
        tickLine="y"
        gridAxis="y"
        withLegend
