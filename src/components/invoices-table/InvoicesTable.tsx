...
const InvoicesTable = ({ data, error, loading }: InvoicesTableProps) => {
  ...
  const columns: DataTableProps<InvoiceTableRow>['columns'] = [
      ...
      render: ({ full_name, email }: InvoiceTableRow) => {
        ...
        return (
          <HoverCard shadow="md" openDelay={500} closeDelay={500}>
            ...
            <HoverCard.Dropdown>
              <Stack gap="xs">
                <Text fz="sm">Имя: {firstName}</Text>
                <Text fz="sm">Фамилия: {lastName}</Text>
    ...
    },
    {
      ...
      filter: (
        <MultiSelect
          label="Статус"
          description="Показать все счета с указанным статусом"
          data={statuses}
    ...
    },
    {
      ...
      render: (item: InvoiceTableRow) => (
        <Group gap="sm">
          ...
          <Tooltip label="Просмотр деталей счёта">
            <ActionIcon
              onClick={() =>
  ...
  return error ? (
    <ErrorAlert title="Ошибка загрузки счетов" message={error.toString()} />
  ) : (
